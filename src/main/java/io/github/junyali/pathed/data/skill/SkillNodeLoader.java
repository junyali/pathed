	...
	protected void apply(Map<ResourceLocation, JsonElement> objects, @NotNull ResourceManager manager, @NotNull ProfilerFiller profiler) {
		...
		for (Map.Entry<ResourceLocation, JsonElement> entry : objects.entrySet()) {
			...
			try {
				...
				SkillNode node = new SkillNode(fileId, parsed.nameKey(), parsed.descriptionKey(), parsed.category(), parsed.position(), parsed.icon(), parsed.nodeType(), parsed.prerequisites(), parsed.previousNodes(), parsed.requirements(), parsed.rewards(), parsed.pathLocked());
